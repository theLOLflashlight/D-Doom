...
void Game::render()
{
    ...
    const mat4 view = lookAt( eyepos, vec3( 0, 0, 0 ), vec3( 0, 1, 0 ) );
    ...
    float distance = eyepos.y - 0;
    eyepos.y -= 2 * distance;
    draw_scene( lookAt( eyepos, vec3( 0, 0, 0 ), vec3( 0, 1, 0 ) ), proj );
    glDisable( GL_CLIP_DISTANCE0 );
    glBindTexture( GL_TEXTURE_2D, _water_textures[ 0 ] );
    glBindFramebuffer( GL_FRAMEBUFFER, 0 );
